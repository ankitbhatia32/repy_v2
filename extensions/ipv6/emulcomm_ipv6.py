...
def _is_valid_ipv6_address(ipaddr):
  """
  <Purpose>
    Determines if ipaddr is a valid IPv6 address.
...
# Public interface
# This is similar to gethostbyname in IPv4 for resolving hostname
def getAddripv6(name):
  """
   <Purpose>
      Provides information about a hostname. Calls socket.getaddrinfo() instead of socket.gethostbyname().
      Translate a host name to IPv6 address format. The IPv4 address is
      returned as a list, such as [(30, 2, 17, '', ('2404:6800:4009:806::2004', 0, 0, 0))]. If the host name is an
  ...
  if type(name) is not str:
    raise RepyArgumentError("getaddrinfo() takes a string as argument.")
  ...
  try:
    return socket.getaddrinfo(name, None, socket.AF_INET6) #getaddrinfo() takes atleast two arguments
...
def getmyip_ipv6():
  ...
  if user_ip_interface_preferences_ipv6:
    update_ip_cache_ipv6()
    # Return the first allowed ip, there is always at least 1 element (loopback)
    return allowediplist_ipv6[0]
  ...
  for ip_addr in repy_constants.STABLE_PUBLIC_IPS_IPv6:  
    try:
      ...
      if _is_valid_ipv6_address(myip): 
        return myip
...
def _get_localIP_to_remoteIP(connection_type, external_ip, external_port=80):
  ...
  """
  # Open a socket
  sockobj = socket.socket(socket.AF_INET6, connection_type)
...
def sendmessage_ipv6(destip, destport, message, localip, localport):
  ...
  if not _is_valid_ipv6_address(destip):
    raise RepyArgumentError("Provided destip is not valid! IP: '"+destip+"'")
  if not _is_valid_ipv6_address(localip):
  ...
  update_ip_cache_ipv6()
  if not _ip_is_allowed_ipv6(localip):
